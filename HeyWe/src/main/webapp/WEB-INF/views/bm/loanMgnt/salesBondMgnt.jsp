...
<html>
	  ...
	  <link rel="stylesheet" type="text/css" href="resources/css/common/calendar.css" />
		...
		$(document).ready(function() {
			...
			$("tbody").on("click", "td", function() {
				...
				$("#salesBondMgntTable tbody input[type='checkbox']").each(function() {
					if(!$(this).is(":checked")) {
						checkFlag++;
					}
				});
			...
			$("tbody").on("click", "[type='checkbox']", function() {
				...
				$("#btn_newReg").on("click", function()	{
					makePopup(1, "매출채권 신규등록", loanNewRegPopup(), true, 500, 500, function() {
						...
						if($.trim($("#dlineDay").val()) == "") {
							...
							makeAlert(2, "알림", "채권금액을 입력하세요", true, null);
							$("#loanAmt").focus();
						} else {
		...
		$("#updateBtn").on("click", function()	{
			var checkFlag = 0;
			$("#salesBondMgntTable tbody input[type='checkbox']:checked").each(function() {
				checkFlag++;
			});
			
			if(checkFlag == 1) {
				// 수정 makePopup
				makePopup(1, "매출채권 수정", salesBondUpdatePopup(), true, 500, 500, function() {
					// datepicker 할당(calendar)
					$("#dlineDay").datepicker({
						dateFormat : 'yy-mm-dd',
						duration: 200,
						...
						success : function(result) {
							...
							$.ajax({
								...
								success : function(result) {
									...
									$("#sltedSalesBondNo").val($("#salesBondMgntTable tbody input[type='checkbox']:checked").val());
									var params = $("#popupDataForm").serialize();
		...
		$("#delBtn").on("click", function() {
			...
			if(checkFlag ==1){
				...
				$("#sltedDelSalesBondNo").val($("#salesBondMgntTable tbody input[type='checkbox']:checked").val());	

								var params = $("#delDataForm").serialize();
								
								$.ajax({
									type : "post",
									url : "BMSalesBondDelAjax",
									dataType : "json",
									data : params,
									success : function(result){
										if(result.flag == 0){
		...
		$("#btn_rpayRtn").on("click",function(){
			...
			if(checkFlag == 1){
			// 상환 makePopup
				makePopup(1, "매출채권 반환", salesBondRpayPopup(), true, 400, 200, function(){
					
					// datepicker 할당(calendar)
					$("#rpayRtnDay").datepicker({
						dateFormat : 'yy-mm-dd',
						duration: 200,
						minDate: $("#salesBondMgntTable input[type='checkbox']:checked").parent().parent().children().eq(1).children().children().html().replace(/[/]/g, '-'),
						maxDate: new Date,
						onSelect:function(dateText, inst){
					...
					$("#sltedRpayRtnNo").val($("#salesBondMgntTable tbody input[type='checkbox']:checked").val());						
				
				}, "반환",function(){
					if($.trim($("#rpayRtnDay").val()) == "") {
						makeAlert(2, "알림", "반환일을 입력하세요", true, null);
						$("#rpayRtnDay").focus();
					} else if($.trim($("#rpayRtnAmt").val()) == "") {
						makeAlert(2, "알림", "반환금액을 입력하세요", true, null);
						$("#rpayRtnAmt").focus();
					} else {
						var params = $("#popupDataForm").serialize();
		...
		// 날짜 클릭으로 상세보기
		$("#salesBondMgntTable").on("click", "#dtlHist", function()	{					
					...
					success : function(result) {
						if(result.list.length != 0){
							...
							for(var i = 0; i < result.list.length; i++) {
								html +=	cnt + ". 반환일 : " + result.list[i].RPAY_DAY + "        작성일 : " + result.list[i].WRITE_DAY + "\n" +
								        "   작성자 : " + result.list[i].NAME + "            반환금액 : " + result.list[i].AMT + "\n\n";
								cnt ++;        
							}
							$("#rpayHist").val(html);
							}
		...
		<%-- 적요명(셀렉트바) 가져오기 --%>
			...
			<%-- 매출채권 리스트 가져오기 --%>
			function reloadSalesBondMgntList() {
				var params = $("#actionForm").serialize();
				...
				$.ajax({
					...
					data : params,
					success : function(result) {
						redrawSalesBondMgntList(result.list);
			...
			function redrawSalesBondMgntList(list) {
				var html = "";
				...
				if(list.length == 0) {
					...
					for(var i = 0; i < list.length; i++) {
						if(i%2 == 0) {
							...
							html += "<td>" + list[i].TOTAL_PSTV_COLLECT_AMT + "</td>";
							html += "<td>" + list[i].TOTAL_NSTV_COLLECT_AMT + "</td>";
							html += "</tr>";
						} else {
				...
				$("#salesBondMgntTable tbody").html(html);
			}
			...
			function loanNewRegPopup() {
				...
				html += "<div class=\"regPage_contents_title\">채권금액(*)</div>";
				html += "<div class=\"regPage_contents_input\">";
			...
			// 수정 팝업 내용
			function salesBondUpdatePopup() {
				var html = "";
				
				html += "<form action=\"#\" method=\"post\" id=\"popupDataForm\">";
				html += "<input type=\"hidden\" id=\"sltedSalesBondNo\" name=\"sltedSalesBondNo\" value=\"\" />";
				html += "<div class=\"regPage_contents_title\">마감날짜(*)</div>";
				html += "<div class=\"regPage_contents_input\">";
				html += "<input type=\"text\" class=\"txt\" id=\"dlineDay\" name=\"dlineDay\" value=\"\" readonly=\"readonly\" />";
				html += "</div>";
			...
			function dtlHistPopup() {
				...
				html += "<div class=\"regPage_contents_title\">반환내역</div>";
				html += "<div class=\"regPage_contents_input\">";
   ...
   <body>
		...
		<form action="#" method="post" id="delDataForm">
			<input type="hidden" id="sltedDelSalesBondNo" name="sltedDelSalesBondNo" value="" />
		</form>
      ...
      <div class="content_area">
         <!-- 메뉴 네비게이션 -->
         <div class="content_nav">HeyWe &gt; 경영관리 &gt; 대출 관리 &gt; 매출채권 관리</div>
         <!-- 현재 메뉴 제목 -->
         <div class="content_title">매출채권 관리</div>
         <br/>
         ...
         <div class="data_info_area">
            ...
            <table class="data_info_table" id="salesBondMgntTable">
               <colgroup>
                  <col width="50"/>
                  <col width="150"/>
                  <col width="100"/>
         ...
         <div class="data_btn_area">
            ...
            <div class="right_btn_area">
               <div class="btn_area"><input type="button" value="반환하기" class="btn" id="btn_rpayRtn"/></div>
